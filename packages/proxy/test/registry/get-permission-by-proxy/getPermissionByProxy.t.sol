...
contract GetPermissionByProxy_Test is Registry_Test {
    function setUp() public virtual override {
        Registry_Test.setUp();
        proxy = registry.deploy();
    }

    function test_GetPermissionByProxy_EnvoyDoesNotHavePermission() external {
        bool permission =
    ...
    function test_GetPermissionByProxy() external whenEnvoyHasPermission {
        bool permission =
            registry.getPermissionByProxy({ proxy: proxy, envoy: users.envoy, target: address(targets.basic) });
