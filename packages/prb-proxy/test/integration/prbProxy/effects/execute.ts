    ...
    context("when the target is a contract", function () {
      context("when the gas stipend calculation results into an underflow", function () {
        ...
        it("reverts", async function () {
          ...
          const data: string = this.contracts.targets.echo.interface.encodeFunctionData("echoUint256", [Zero]);
      ...
      context("when the gas stipend calculation does not result into an underflow", function () {
        context("when the owner was changed during the delegate call", function () {
          it("reverts", async function () {
            const target: string = this.contracts.targets.changeOwner.address;
            const data: string = this.contracts.targets.changeOwner.interface.encodeFunctionData("changeOwner");
            await expect(this.contracts.prbProxy.connect(owner).execute(target, data)).to.be.revertedWith(
            ...
            context("when ether is sent", function () {
              it("returns the ether amount back", async function () {
                ...
                const response: string = await this.contracts.prbProxy
                  .connect(owner)
                  .callStatic.execute(this.contracts.targets.echo.address, data, { value: sentAmount });
            ...
            context("when ether is not sent", function () {
              context("when the target self destructs", function () {
                it("returns an empty response and sends the ether to the recipient", async function () {
                  ...
                  const data: string = this.contracts.targets.selfDestruct.interface.encodeFunctionData("destroyMe", [
                    AddressZero,
                  ]);
                  const response: string = await this.contracts.prbProxy
                    .connect(owner)
                    .callStatic.execute(this.contracts.targets.selfDestruct.address, data);
              ...
              context("when the target does not self destruct", function () {
                ...
                it("returns the response as address", async function () {
                  ...
                  const data: string = this.contracts.targets.echo.interface.encodeFunctionData("echoAddress", [input]);
                  const response: string = await this.contracts.prbProxy
                    .connect(owner)
                    .callStatic.execute(target, data);
                  const decodedResponse: string = this.contracts.targets.echo.interface
                ...
                it("returns the response as array", async function () {
                  ...
                  const data: string = this.contracts.targets.echo.interface.encodeFunctionData("echoArray", [input]);
                  const response: string = await this.contracts.prbProxy
                    .connect(owner)
                    .callStatic.execute(target, data);
                ...
                it("returns the response as bytes", async function () {
                  ...
                  const data: string = this.contracts.targets.echo.interface.encodeFunctionData("echoBytes", [input]);
                  const response: string = await this.contracts.prbProxy
                    .connect(owner)
                    .callStatic.execute(target, data);
                  const decodedResponse: string = this.contracts.targets.echo.interface
                ...
                it("returns the response as bytes32", async function () {
                  ...
                  const data: string = this.contracts.targets.echo.interface.encodeFunctionData("echoBytes32", [input]);
                  const response: string = await this.contracts.prbProxy
                    .connect(owner)
                    .callStatic.execute(target, data);
                ...
                it("returns the response as string", async function () {
                  ...
                  const data: string = this.contracts.targets.echo.interface.encodeFunctionData("echoString", [input]);
                  const response: string = await this.contracts.prbProxy
                    .connect(owner)
                    .callStatic.execute(target, data);
                  const decodedResponse: string = this.contracts.targets.echo.interface
                ...
                it("returns the response as struct", async function () {
                  ...
                  const data: string = this.contracts.targets.echo.interface.encodeFunctionData("echoStruct", [input]);
                  const response: string = await this.contracts.prbProxy
                    .connect(owner)
                    .callStatic.execute(target, data);
                ...
                it("returns the response as uint8", async function () {
                  ...
                  const data: string = this.contracts.targets.echo.interface.encodeFunctionData("echoUint8", [input]);
                  const response: string = await this.contracts.prbProxy
                    .connect(owner)
                    .callStatic.execute(target, data);
                ...
                it("returns the response as uint256", async function () {
                  ...
                  const data: string = this.contracts.targets.echo.interface.encodeFunctionData("echoUint256", [input]);
                  const response: string = await this.contracts.prbProxy
                    .connect(owner)
                    .callStatic.execute(target, data);
                  expect(input).to.equal(bn(response));
