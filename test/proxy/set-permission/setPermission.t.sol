...
contract SetPermission_Test is Proxy_Test {
    ...
    function test_RevertWhen_CallerNotOwner() external {
        ...
        proxy.setPermission({ envoy: caller, target: address(targets.dummy), permission: true });
    }
    ...
    function test_SetPermission_PermissionNotSet() external callerOwner {
        proxy.setPermission({ envoy: users.envoy, target: address(targets.dummy), permission: true });
    ...
    function test_SetPermission_PermissionSet_UnsetPermission() external callerOwner permissionSet {
        proxy.setPermission({ envoy: users.envoy, target: address(targets.dummy), permission: false });
    }
